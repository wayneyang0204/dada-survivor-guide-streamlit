...
def classify_article(title: str, excerpt: str = "") -> str:
    ...
    if "區域行動" in title:
        return "關卡模式"
    rules = [
        ("活動攻略", ("活動", "慶典", "派對", "扭蛋", "拼圖", "探寶", "尋寶", "格子舖", "礦場", "彩虹礦", "彩虹棋", "彩虹骰", "一番賞", "訂單員", "轉盤", "圓盤", "麥克風", "大亂鬥", "水槍", "鉛筆", "橡皮", "金秋海岸", "釣魚")),
        ("載具養成", ("載具", "模組", "同步率", "平衡車", "懸浮滑板")),
        ("角色特工", ("特工", "角色", "覺醒", "同調", "協同", "碎片", "洛基", "伊狑")),
        ("寵物系統", ("寵物", "異世", "異寵", "助戰")),
...
def fetch_source_posts(max_pages: int = 2, per_page: int = 100) -> list[dict[str, Any]]:
    ...
    for page in range(1, max_pages + 1):
        ...
        for post in batch:
            ...
            title = title.replace("SP", "神火").replace("sp", "神火")
...
EVENT_PLAYBOOKS = [
    {
        "name": "金秋海岸狂歡",
        "keywords": ("金秋海岸",),
        "mechanic": "官方活動卡確認：完成任務取得魚餌，釣魚可解鎖新圖鑑；釣魚積分可兌換獎勵。其他規則仍需在遊戲內核對。",
        "target": 0,
        "unit": "釣魚積分",
        "free_hint": "官方尚未公布可核對的免費魚餌總量；先領取遊戲內任務魚餌，並逐日記錄實際取得量。",
        "verdict": "先拿免費魚餌並查看積分商店；沒有可靠的固定停損線前，不為未知獎勵提前買魚餌。",
        "period": "官方活動卡已上線；截止時間以遊戲內倒數為準",
        "tags": ["官方活動", "魚餌任務", "釣魚圖鑑", "積分商店", "門檻待核對"],
        "highlights": [
            ("已確認玩法", "任務取得魚餌、解鎖釣魚圖鑑、釣魚積分兌換獎勵。"),
            ("尚待核對", "魚餌總量、積分門檻、商店兌換價值與截止時間。"),
            ("現在先做", "領免費魚餌，記錄實際積分與商店目標，再填入下方試算。"),
        ],
        "steps": ["先完成遊戲內免費任務，領取魚餌", "核對釣魚圖鑑與積分商店中真正需要的獎勵", "確認門檻和倒數後才試算差額", "沒有把握跨過高價值門檻就保留寶石"],
        "avoid": "不要把其他釣魚活動的魚餌回收率或積分停損線直接套用到本期。",
    },
    {
        "name": "鉛筆王國秘寶",
        "keywords": ("鉛筆王國", "鉛筆", "橡皮"),
        "mechanic": "每次行動會隨機讓一至四欄下降一格；只要任一頂部獎勵被推出，就立刻重置，把下一輪成本維持在一張票。下排獎勵較易推出，上排較難但數量加倍。",
        "target": 5250,
        "unit": "累計積分",
        "free_hint": "社群任務表合計五百八十五張票，但其中三百六十張來自開到六百個寶箱，不能把鑰匙成本當成免費。一般建議先抓三百至三百六十箱。",
        "verdict": "本期已於 9 月 21 日結束；五千二百五十積分只保留作復刻核對，不能當成現行活動。",
        "period": "社群整理：9/17 00:00－9/21 23:59（以遊戲內倒數為準）",
        "tags": ["歷史活動", "五千二百五十停損", "立即重置", "開箱成本", "不追排名"],
        "highlights": [
            ("推薦停損", "累計五千二百五十積分，拿諧振晶片與傳奇收藏品自選箱後停手。"),
            ("擺放順序", "下排放補給箱、配件與收藏品鑰匙；上排放史詩配件或傳奇收藏品碎片。"),
            ("開箱節奏", "前四天先開二百至二百六十箱，最後一天再依積分補到約三百至三百六十箱。"),
            ("免費量陷阱", "五百八十五張包含開到六百個寶箱的任務，並不是零成本票券。"),
        ],
        "steps": ["先把高價、數量加倍的獎勵放上排", "只要頂部獎勵被推出就立刻重置", "前四天先開二百至二百六十箱", "最後一天只補到五千二百五十積分，達標即停"],
        "avoid": "不要為已升級但位置不佳的獎勵承擔六張票成本；不追六千六百解構機，也不買票搶每日排名。",
    },
    {
        "name": "水上樂園大亂鬥",
        "keywords": ("水上樂園", "水槍"),
        "mechanic": "長按可自動發射水槍；擊敗特殊發光敵人可解鎖海盜船長。水槍會隨機返還，返還幅度屬社群試算，並非官方保證。",
        "target": 330,
        "unit": "累計消耗水槍",
        "free_hint": "社群估算完成全部任務可取得三百一十八至三百二十二把水槍，但其中包含開寶箱任務，鑰匙不能算免費；隨機返還約三至五成也僅供估算。",
        "verdict": "本期已於 9 月 15 日結束；三百三十累計水槍只保留作復刻核對，不能當成現行活動。",
        "period": "社群整理：9/11 00:00－9/15 23:59（以遊戲內倒數為準）",
        "tags": ["歷史活動", "三百三十停損", "神器核心", "水槍返還", "開箱成本"],
        "highlights": [
            ("推薦停損", "累計消耗三百三十水槍，拿神器核心自選箱後停手。"),
            ("開箱節奏", "前四天先開二百至二百五十箱，最後一天依水槍返還與差額補到常見總量二百五十至三百五十箱。"),
            ("免費量陷阱", "三百一十八至三百二十二把包含開箱任務，不能把鑰匙成本忽略。"),
            ("商店優先", "高階帳號先換傳奇載具碎片自選箱、萬能神火碎片，再換載具材料與模組。"),
        ],
        "steps": ["先完成每日任務並記錄水槍返還", "前四天開二百至二百五十箱", "最後一天只補到累計三百三十", "拿到神器核心後停手，再依載具碎片、神火碎片、載具材料與模組順序換商店"],
        "avoid": "不追六百三十檔；社群估算即使開六百箱，仍可能需要二萬二至五萬寶石。",
    },
    {
        "name": "煲湯廚房",
        "keywords": ("煲湯廚房", "湯勺", "砂鍋"),
    ...
    {
        "name": "神火特攻",
        "keywords": ("神火特攻", "伏爾甘", "哪吒", "洛基"),
        "mechanic": "神火特工保底型高消耗活動；洛基偏向協同支援，單獨連攜時不能自行產生神火。哪吒覺醒6搭配洛基覺醒4，才可持續銜接神火與餘燼。",
        "target": 300,
        "unit": "累計抽數",
        "free_hint": "洛基活動三百抽保底；社群估算十二天免費票約一百至一百一十七張，常見寶石缺口約五萬五至六萬，仍應以遊戲內任務表為準。",
        "verdict": "哪吒未達覺醒6且寶石不足六萬就跳過；已有哪吒覺醒6，洛基覺醒4是第一個投入目標。",
        "period": "洛基上線後玩家實測：2026/09/17（活動倒數以遊戲內為準）",
        "tags": ["玩家實測", "洛基", "哪吒覺醒6", "洛基覺醒4", "三百抽保底"],
        "steps": ["先確認哪吒是否已到覺醒6", "把免費票與寶石一起換算成三百抽缺口", "符合條件才把洛基做到覺醒4", "資源不足完整斷點就跳過本期"],
        "avoid": "不要把洛基當上場主位，也不要沿用 9 月 15 日上線前『單靠洛基就能常駐神火』的推測。",
    },
    {
        "name": "王牌訂單員",
